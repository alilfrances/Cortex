...
6. Use `cortex_relations` for parsed one-hop graph questions such as imports, contains, inherits, emits, connects, handles, QML `instantiates`, or CMake/QRC `builds`/`registers` wiring.
7. Use `cortex_path` when the question is multi-hop — how one symbol reaches another through calls, containment, imports, or Qt wiring.
8. Use `cortex_references` when configs, docs, scripts, CMake/QRC, or other parser-missed surfaces may reference a symbol. Pass `mode: "writes"` when you only need definitions and mutation sites.
9. Use `cortex_search_text` for body text — string literals, error messages, comments, Markdown prose — that `cortex_search_symbols` can't find because it only matches symbol names/signatures/paths, not file contents.
10. Before a commit or review, call `cortex_risk` with `range` (or `staged: true`) to get deterministic per-file scores and missing co-change/test/Qt/build-reference directives. It is local-only and can still report a clearly marked partial result when no Cortex index exists.
11. Use `cortex_dead_code` for deterministic dead-code candidates from graph fan-in plus grep references; treat `high` as the strictest tier, and remember that regex-backend languages and Qt meta-object surfaces are conservatively capped or excluded.
12. Default to `response_format: "concise"`; pass `response_format: "detailed"` only when you need provenance, fingerprints, full metadata, or detailed why-edges.
13. If a tool reports `stale: true`, call `cortex_refresh` or rerun the read tool after refresh.
14. Watch for a `_meta` object on any response (`index_age_seconds`, `indexed_at`, `fingerprint_fresh`, and optionally `auto_refreshed`/`cached`/`saved_tokens`). `detailed` responses always carry it; a `concise` response only carries it when something is worth acting on, so its mere presence is itself a signal — check `fingerprint_fresh`/`auto_refreshed` before trusting a concise result on a repo you suspect just changed.
...
Returns parsed graph edges filtered by relation and symbol. Use for structural questions where parser coverage is enough, including Qt `emits`/`connects`/`handles`, QML `instantiates`, and CMake/QRC `builds`/`registers` edges.
...
Returns up to three shortest structural paths between two resolved symbols. Use it for multi-hop flow questions after locating both endpoints; commit nodes and co-change edges are excluded so history cannot create a misleading shortcut.

Example:

```json
{"symbol_a":"generate_bundle","symbol_b":"count_text_tokens","max_depth":6}
```

### `cortex_references`

Returns graph plus grep references bucketed by file type. Use for cross-language or config/doc/script blast radius. The default `mode: "all"` tags each hit as a definition, read, or write; `mode: "writes"` filters out reads so mutation questions do not require manual triage.

Example:

```json
{"symbol":"_ensure_fresh","mode":"writes","budget":2000}
```
